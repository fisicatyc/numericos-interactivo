...
def thrnodeopn(lowerlimit, upperlimit, function, flag = 0):
    ...
    if flag == 0:
        ...
        integ = 5*h/24*(11*function(lowerlimit) + function(lowerlimit + h) + \
                function(lowerlimit + 2*h) + 11*function(lowerlimit + 3*h));
    else:
        if len(function) > 5:
            raise Exception("Low order function is suggested");
        
        integ = 5*h/24*(11*function[0] + function[1] + function[2] + \
                11*function[3]);
